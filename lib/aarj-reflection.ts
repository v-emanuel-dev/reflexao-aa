...
function parseAarjReflection(html: string): Reflection {
  ...
  const dateLabel = cleanText(
    $('h3.text-primary')
      .filter((_, element) => /^\d{2}\/\d{2}$/.test(cleanText($(element).text())))
      .first()
      .text()
  )
  ...
  if (!title || !dateLabel || !excerpt || body.length === 0) {
    throw new Error('Não foi possível extrair título, data, chamada ou corpo da reflexão da AARJ.')
  }
  ...
  return {
    id: `${year}-${month}-${day}`,
    slug: slugify(title),
    title: title.toLocaleLowerCase('pt-BR').replace(/^./, (letter) => letter.toLocaleUpperCase('pt-BR')),
    dateLabel,
...
export async function getDailyReflection(): Promise<ReflectionFetchResult> {
  try {
    const response = await fetch(AARJ_REFLECTION_URL, {
      ...
      headers: {
        'user-agent': 'Mozilla/5.0 ReflexaoDiariaModern/1.0 (+https://vercel.app)',
